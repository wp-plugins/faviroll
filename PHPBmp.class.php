...
require_once('php_thumb.functions.php');
class PHPBmp {
	 ...
	 *
	 */
	function bmp2gd(&$BMPdata, $truecolor=true) {
		$ThisFileInfo = array();
	 ...
	 *
	 */
	function bmpfile2gd($filename, $truecolor=true) {
		if ($fp = @fopen($filename, 'rb')) {
			$BMPdata = fread($fp, filesize($filename));
			fclose($fp);
			return $this->bmp2gd($BMPdata, $truecolor);
		}
		return false;
	}
